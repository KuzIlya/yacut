...
def test_duplicated_url_in_form(client, short_python_url, duplicated_custom_id_msg):
    response = client.post(
        INDEX_URL,
        data={
            "original_link": PY_URL,
...
def test_get_unique_short_id(client):
    ...
    assert response.status_code == HTTPStatus.OK, asser_msg_pattern.format(
        f"должн вернуть статус-код `{HTTPStatus.OK.value}`"
    )
    url_map_object = URLMap.query.filter_by(original=PY_URL).first()
    assert url_map_object, asser_msg_pattern.format(
        "в базе данных должна создаваться новая запись"
    )
    assert url_map_object.short, asser_msg_pattern.format(
